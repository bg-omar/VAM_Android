...
export class AccountPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.userSettings = [...this.userSettings, this.addVam]
  }
  ...
  setJson(setter: number) {
    if (setter == 1) {
      ...
      if (this.getterdata.find(value => value.ipAddress === ip[0].ipAddress)) {
        ...
        console.log("%c 2 --> 114||C:/workspace/projects/constants-calculator/src/app/pages/account/account.ts\n this.user.ipAddress: ", "color:#0f0;", this.user.ipAddress);
      } else {
        this.getterdata = [
          ...this.getterdata,
          ...ip
      ...
      this.storageServive.setData("VAM", configAccount).then(r => {
      });
    ...
    if (getter == 1) {
      await this.storageServive.getData("VAM").then((data: any) => {
        ...
        if (data.value) {
          ...
          console.log("%c 1 --> 135||C:/workspace/projects/constants-calculator/src/app/pages/account/account.ts\n this.getterdata: ", "color:#f0f;", this.getterdata);
        }
      });
    }
  ...
  async delJson(i) {
    await this.storageServive.delData("VAM");
  ...
  async getPass(x) {
    if (x === 1) {
      if (Capacitor.getPlatform() === 'android') {
        ...
        this.vamPass = result.value || 'no password'
